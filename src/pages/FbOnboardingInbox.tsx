...
const STATUS_LABEL: Record<FbOnboardingStatus, string> = {
  recu: "Reçu",
  traite: "Traité",
  archive: "Archivé",
};

const STATUS_COLOR: Record<FbOnboardingStatus, string> = {
...
export default function FbOnboardingInbox() {
  ...
  const filtered = (data || []).filter((s) => {
    ...
    return (
      ...
      s.business_manager_email.toLowerCase().includes(q) ||
      (s.business_manager_id || "").includes(q)
    );
  });
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2">
          <Inbox className="h-7 w-7 text-[#ff006e]" /> Onboarding Facebook
        </h1>
        <p className="text-muted-foreground mt-1">
          Soumissions reçues via le tutoriel public <code className="text-xs bg-muted px-1.5 py-0.5 rounded">/tuto/facebook</code>
        </p>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <Input
            placeholder="Rechercher (entreprise, email, BM ID...)"
            value={search}
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center text-muted-foreground">
            Aucune soumission {filter !== "all" ? `(${STATUS_LABEL[filter as FbOnboardingStatus]})` : ""} pour le moment.
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={s.id}>
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Building2 className="h-5 w-5 text-[#ff006e]" />
                      {s.company_name}
                      {s.client_ndi && (
                        <Badge variant="outline" className="text-xs">{s.client_ndi}</Badge>
                      )}
                    </CardTitle>
                    <p className="text-xs text-muted-foreground mt-1">
                      Reçu le {format(new Date(s.created_at), "d MMM yyyy 'à' HH:mm", { locale: fr })}
                      {!s.has_existing_page && (
                        <span className="ml-2 text-amber-600 font-semibold">• Page créée pour l'occasion</span>
                      )}
                    </p>
                  </div>
              ...
              <CardContent className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3 text-sm">
                  ...
                  {s.business_manager_id && (
                    <InfoRow icon={KeyRound} label="ID Business Manager" value={s.business_manager_id} mono />
                  )}
                  {s.fb_page_name && (
                    <InfoRow icon={Facebook} label="Nom de la page" value={s.fb_page_name} />
                  )}
                  {s.fb_page_url && (
                    <InfoRow icon={Link2} label="URL de la page" value={s.fb_page_url} link />
                  )}
                </div>
                ...
                <div className="flex gap-2 flex-wrap pt-2">
                  {s.status !== "traite" && (
                    <Button
                      size="sm"
                      onClick={() => updateStatus.mutate({ id: s.id, status: "traite" })}
                  ...
                  {s.status !== "archive" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateStatus.mutate({ id: s.id, status: "archive" })}
                    >
                  ...
                  {s.client_id && (
                    <Button size="sm" variant="outline" asChild>
                      <a href={`/clients/${s.client_id}`}>
                        <ExternalLink className="h-4 w-4" /> Voir la fiche client
                      </a>
                    </Button>
                  )}
                  <Button
                    ...
                    onClick={() => {
                      if (confirm("Supprimer cette soumission ?")) remove.mutate(s.id);
                    }}
                  >
        ...
        <div className="flex items-center gap-1.5">
          {link ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`truncate text-[#ff006e] hover:underline ${mono ? "font-mono" : ""}`}
            >
              {value}
            </a>
          ) : (
          ...
          <button
            ...
            className="text-muted-foreground hover:text-[#ff006e] flex-shrink-0"
            title="Copier"
          >
