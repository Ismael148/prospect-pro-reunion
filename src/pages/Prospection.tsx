...
import pagesjaunesLogo from "@/assets/logos/pagesjaunes.png";
import tripadvisorLogo from "@/assets/logos/tripadvisor.png";
import yelpLogo from "@/assets/logos/yelp.png";

const PLATFORM_MAP: Record<string, { logo: string; label: string }> = {
  "google.com/maps": { logo: googleMapsLogo, label: "Google Maps" },
  "goo.gl/maps": { logo: googleMapsLogo, label: "Google Maps" },
  "facebook.com": { logo: facebookLogo, label: "Facebook" },
  "instagram.com": { logo: instagramLogo, label: "Instagram" },
  "linkedin.com": { logo: linkedinLogo, label: "LinkedIn" },
  "pagesjaunes.fr": { logo: pagesjaunesLogo, label: "Pages Jaunes" },
  "tripadvisor": { logo: tripadvisorLogo, label: "TripAdvisor" },
  "yelp.com": { logo: yelpLogo, label: "Yelp" },
};

function getPlatformInfo(url: string): { icon: React.ReactNode; label: string } {
  const u = url.toLowerCase();
  for (const [domain, info] of Object.entries(PLATFORM_MAP)) {
    if (u.includes(domain)) {
      return {
        icon: <img src={info.logo} alt={info.label} className="w-4 h-4 rounded-sm object-contain" loading="lazy" width={16} height={16} />,
        label: info.label,
      };
    }
  }
  return { icon: <ExternalLink className="w-4 h-4 text-muted-foreground" />, label: "Site web" };
}
